...
import {
  generateChromeExtensionIcons,
  buildManifestIconsSnippet,
} from "@/lib/chrome-extension-icons/generate";
import { chromeExtensionIconTargets } from "@/lib/chrome-extension-icons/sizes";
import {
  clearImageHandoff,
  readImageHandoff,
...
export default function ChromeExtensionIconsPage() {
  ...
  const loadFile = useCallback(
    (file: File, fromHandoff = false) => {
      ...
      img.onload = () => {
        ...
        if (longest < MIN_SOURCE_SIZE) {
          URL.revokeObjectURL(url);
          setError(
            `Image must be at least ${MIN_SOURCE_SIZE}px on its longest side. Yours is ${img.naturalWidth}×${img.naturalHeight}.`,
          );
          return;
        }
        setSourceImage(img);
  ...
  const getSourceArtifact = useCallback(async () => {
    ...
    return {
      ...
      sourceHref: "/tools/chrome-extension-icons",
    };
  }, [sourceFile]);
  ...
  const handleCopy = useCallback(async () => {
    await navigator.clipboard.writeText(buildManifestIconsSnippet());
    setCopied(true);
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground mt-1">
        Upload a PNG (at least 128px) and download every icon size a Chrome
      ...
      >
        {sourceUrl ? (
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-muted-foreground text-xs">
              At least 128px on the longest side
            </p>
          </div>
        )}
        <input
      ...
      {zipBlob && (
        <div className="mt-10">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold">manifest.json snippet</h2>
          ...
          <pre className="bg-muted mt-3 overflow-x-auto rounded-lg p-4 text-sm">
            <code>{buildManifestIconsSnippet()}</code>
          </pre>
        </div>
      )}
    </div>
