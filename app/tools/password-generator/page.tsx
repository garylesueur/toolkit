...
function generatePassword(length: number, alphabet: string): string {
  ...
  const maxValid =
    Math.floor(256 ** bytesPerChar / alphabet.length) * alphabet.length;

  let result = "";
  while (result.length < length) {
    const bytes = crypto.getRandomValues(
      new Uint8Array((length - result.length) * bytesPerChar * 2),
    );
    for (
      let i = 0;
      i <= bytes.length - bytesPerChar && result.length < length;
      i += bytesPerChar
    ) {
      let value = 0;
